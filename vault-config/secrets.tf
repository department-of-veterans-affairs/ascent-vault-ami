...
resource "vault_generic_secret" "gateway-cert" {
  path = "secret/ascent-gateway/ssl"
...
# VBMS SERVICE
# ---------------------------------------------------------------------------------------------------------------------
resource "vault_generic_secret" "vbms-token" {
  path = "secret/vetservices-document"
  data_json = <<EOT
{
  "vbms.saml.token": ${jsonencode(file(var.vbms_saml_token))}
}
EOT
}
